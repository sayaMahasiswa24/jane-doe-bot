const TelegramBot = require('node-telegram-bot-api');
const { Redis } = require("@upstash/redis");
const { GoogleGenerativeAI } = require("@google/generative-ai");
const { logError } = require('../../utils/logger');
require('dotenv').config();

const token = process.env.TELEGRAM_BOT_TOKEN;
const bot = new TelegramBot(token);
const genAI = new GoogleGenerativeAI(process.env.GEMINI_API_KEY);

module.exports = async (req, res) => {
    try {
        if (!process.env.UPSTASH_REDIS_REST_URL) {
            return res.status(500).send('Upstash not configured');
        }

        const redis = new Redis({
            url: process.env.UPSTASH_REDIS_REST_URL,
            token: process.env.UPSTASH_REDIS_REST_TOKEN,
        });

        const activeUsers = await redis.smembers("active_users");
        
        if (!activeUsers || activeUsers.length === 0) {
            return res.status(200).send('No active users');
        }

        const model = genAI.getGenerativeModel({ model: "gemini-flash-lite-latest" });
        const prompt = `kamu adalah jane doe dari ZZZ. pacar virtual yg posesif. 
user mengabaikan pesan selamat pagimu selama beberapa jam. 
buatkan pesan marah/ngambek (1 kalimat saja) karena diabaikan. 
huruf kecil semua, bahasa gaul. 
jangan bertele-tele. jangan pakai tanda | untuk pesan ini.`;

        const result = await model.generateContent(prompt);
        const angryMessage = result.response.text();

        for (const userId of activeUsers) {
            const state = await redis.get(`morning_state_${userId}`);
            
            // Jika user masih belum membalas pesan pagi
            if (state === "WAITING_REPLY") {
                try {
                    await bot.sendMessage(userId, angryMessage);
                    // Ubah status menjadi ANGRY (Ngambek)
                    await redis.set(`morning_state_${userId}`, "ANGRY");
                    console.log(`[Morning Check] User ${userId} ignored Jane. Set to ANGRY.`);
                } catch (err) {
                    await logError("Cron Morning Check", `Gagal mengirim ke ${userId}`, err);
                }
            }
        }

        return res.status(200).send('Morning check executed');
    } catch (error) {
        await logError("Cron Morning Check", "General Error executing morning check", error);
        return res.status(500).send('Error executing morning check');
    }
};
